...
class BitStorage constructor(
    val bitsPerEntry: Int,
    val size: Int,
    data: LongArray? = null
) {
    init {
        require(!(bitsPerEntry < 1 || bitsPerEntry > 32)) { "bitsPerEntry must be between 1 and 32, inclusive." }
    }
    ...
    private val magicIndex = 3 * (valuesPerLong - 1)
    private val divideMultiply = MAGIC_VALUES[magicIndex].toLong() and 0xffffffffL
    private val divideAdd = MAGIC_VALUES[magicIndex + 1].toLong() and 0xffffffffL
    private val divideShift = MAGIC_VALUES[magicIndex + 2]
    ...
    operator fun get(index: Int): Int {
        ...
        return (data[cellIndex] shr bitIndex and maxValue).toInt()
    }
    ...
    operator fun set(index: Int, value: Int) {
        ...
        require(!(value < 0 || value > maxValue)) { "Value cannot be outside of accepted range." }
        val cellIndex = cellIndex(index)
        val bitIndex = bitIndex(index, cellIndex)
        data[cellIndex] =
            (data[cellIndex] and (maxValue shl bitIndex).inv()) or ((value.toLong() and maxValue) shl bitIndex)
    }

    private fun cellIndex(index: Int): Int {
        val result = index * divideMultiply + divideAdd shr 32 shr divideShift
        return result.toInt()
    }
