...
class Logger(
    ...
    constructor(loggingClass: KClass<*>, level: LogLevel = LogLevel.INFO) : this(
        loggingClass.simpleName ?: loggingClass.toString(), level
    )

    fun info(trace: String) = info { trace }
    fun info(trace: () -> String) {
        if (LogLevel.INFO >= level) {
            Log.info(name, trace())
        }
    }

    fun warn(trace: String) = warn { trace }
    fun warn(trace: () -> String) {
        if (LogLevel.WARN >= level) {
            Log.warn(name, trace())
        }
    }

    fun error(trace: String, throwable: Throwable? = null) = error(throwable) { trace }
    fun error(throwable: Throwable? = null, trace: () -> String) {
        if (LogLevel.ERROR >= level) {
            Log.error(name, trace(), throwable)
        }
    }
...
object Log {
    ...
    fun info(logger: String, trace: String) {
    ...
    fun error(logger: String, trace: String, throwable: Throwable?) {
        val time = startTime.elapsedNow()
        val log = buildString {
            append(CYAN)
            appendTime(time)
    ...
    fun warn(logger: String, trace: String) {
        val time = startTime.elapsedNow()
        val log = buildString {
            append(CYAN)
            appendTime(time)
